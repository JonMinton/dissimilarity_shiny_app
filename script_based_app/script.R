...
# n : (lowercase) : the numerator count 
...
w_nb <- poly2nb(shp_with_attributes, row.names = shp_with_attributes@data$datazon)
...
att_dta <- shp_with_attributes@data

w_dz <- rownames(w_nb)
# remove duplicates
w_nb <- w_nb[!duplicated(w_dz), !duplicated(w_dz)]

# remove areas with no neighbours
w_rowsum <- apply(w_nb, 1, sum)
w_nb <- w_nb[w_rowsum > 0, w_rowsum > 0]

w_dz <- rownames(w_nb)
dta_dz <- as.character(shp_with_attributes@data$datazon)

ss <- intersect(w_dz, dta_dz)
tmp <- w_dz %in% ss
w_nb <- w_nb[tmp, tmp]
dta <- subset(shp_with_attributes@data, datazon %in% ss)
